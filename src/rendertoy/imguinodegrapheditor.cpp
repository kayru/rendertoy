...
void NodeGraphEditor::render()
{
	...
	if (showNodeList && ImGui::CollapsingHeader("Node List##node_list_1",NULL,false))   {
        ImGui::Separator();
...
#       endif //NO_IMGUIHELPER_SERIALIZATION
	...
	ImGui::Spacing();
		...
		node->Size.y = std::max(node->Size.y, float(node->InputsCount * (2 + GetFontSize())));
		node->Size.y = std::max(node->Size.y, float(node->OutputsCount * (2 + GetFontSize())));
        ImVec2 node_rect_max = node_rect_min + node->Size;

	// Go backwards and display title bar buttons if node->isOpen
    // BUTTONS ========================================================
    if (false && node->Size.x!=0 && !node->isInEditingMode)    {
...
#   endif //DEBUG_LINK_CULLING
    ...
    ImGui::PopStyleColor(2);	// ImGuiCol_ChildWindowBg, ImGuiCol_Border
    ImGui::PopStyleVar(2);
...
class CombineNode : public Node {
	...
	ThisClass* node = (ThisClass*) ImGui::MemAlloc(sizeof(ThisClass));IM_PLACEMENT_NEW(node) ThisClass();
        ...
        // 2) main init
        node->init("CombineNode",pos,"in1;in2;in3;in4;in5","out",TYPE);
